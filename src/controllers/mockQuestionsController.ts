...
class MockQuestionsController {
  private static generateQuestions(moduleId: string, lessonIndex: number): Question[] {
    ...
    return lessonData.map((q, index) => ({
      ...
      options: q.options,
      correct: q.correct,
      difficulty: (Math.min(index + 1 + lessonIndex, 5)) as 1 | 2 | 3 | 4 | 5
    }));
  }

  private static getLessonData(moduleId: string, lessonIndex: number): Array<{question: string, options: string[], correct: number}> {
  ...
  private static getAllModuleLessons(moduleId: string) {
    const lessons: Record<string, Record<string, Array<{question: string, options: string[], correct: number}>>> = {
  ...
  private static getDefaultQuestions(): Array<{question: string, options: string[], correct: number}> {
    return [
  ...
  static async getLessonQuestions(req: Request, res: Response) {
    try {
      ...
      logUserAction('questions_loaded', 'system', { moduleId, lessonIndex });
  ...
  static async getAllQuestions(req: Request, res: Response) {
    try {
      ...
      // Generate questions for all lessons (1-10) for each module
